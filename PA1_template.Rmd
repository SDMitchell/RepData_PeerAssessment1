...
* The median number of steps per day after imputation of the NA values is $`r imputedMedianStepsPerDay`$ steps. It makes sense that this now matches the mean due to the fact that we have now inserted several new, identical samples just above the old median causing it to shift.
...
```{r}
imputedData <- cbind(imputedData, as.factor(ifelse(as.POSIXlt(imputedData$date)$wd %in% c(0, 6), "Weekend", "Weekday")))
colnames(imputedData)[4] <- "TypeOfDay"

g <- ggplot(imputedData, aes(interval, steps))
